...
Steps = ig.Game.extend({
  clearColor: 	'#008800',
  currentLevel: new LevelTutorial01(),
  curTime: 			0,
  msgTimer: 		0,
  fadeDir: 			-1,
  curMsg: 			null,
  ...
  background: new ig.Image( 'media/art/background.png' ),
	...
	drawMessage: function() {
		if (this.curMsg) {
			...
			this.font.draw(ig.Game._msgQueue[ig.Game._curMsgIndex].text, Constants.MSG_WINDOW_WIDTH / 2, Constants.HEIGHT - 3 * Constants.TILE_DX / 4 - this.font.height / 4, ig.Font.ALIGN.CENTER);
			ctx.globalAlpha = 1;
	...
	updateMessage: function() {
		...
		if (this.msgTimer > 0 && this.fadeDir === 0) {
			...
			if (this.msgTimer <= 0) {
				// Start fade in to next message, if any.
				this.advanceMessage();
			}
		}
		else if (this.fadeDir > 0 && this.msgTimer > 0) {
...
ig.Game.AddMessage = function(text, speaker, duration) {
