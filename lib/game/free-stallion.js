...
FreeStallion = HorseSprite.extend({
  ...
  testDir:          Constants.DIRECTION.NONE,
  ...
  getSpookPattern: function(spookDir) {
    return Constants.PATTERN_RIGHT_THEN_LEFT[spookDir];
  },
  ...
  resolveFreeMove: function(level) {
    ...
    for (var i=0; i<nTests; ++i) {
      this.testDir = pattern ? pattern[i] : this.wantDir;
      ...
      if (this.isMoving()) {
        this.spookAnimalsInDirection(level, Constants.MOVE_SPOOK_RADIUS, this.testDir);
      }
      ...
      if (level.isMovingDir(this.testDir) && this.trueGridPos.row > 0) {
      ...
      if (this.collisionResults.bDidCollide) {
        ...
        var bNearCollide = level.collideWithSceneElements(this, this.resolveRow(newY + Constants.TILE_DY / 2), this.resolveCol(newX + Constants.TILE_DX / 2), true, this.testDir);
        var bFarCollide = level.collideWithSceneElements(this, this.resolveRow(newY + Constants.TILE_DY / 2 + dynamicLeadY), this.resolveCol(newX + Constants.TILE_DX / 2 + dynamicLeadX), false, this.testDir);
    ...
    if (blockType === Constants.BLOCK_TYPE.HARD) {
      ...
      if (level.isMovingDir(this.testDir)) {
        this.setDirection(this.testDir);
      }

      if (bWantsFlip && (this.testDir == Constants.DIRECTION.UP || Constants.DIRECTION.DOWN)) {
    ...
    level.addToScene(this, this.getRow(true), this.getCol(true));
  ...
  getDebugColor: function() {
    ...
    if (this.collisionResults && this.collisionResults.bDidCollide) {
      color = "#FF8800";
    }
    else {
      switch (this.state) {
